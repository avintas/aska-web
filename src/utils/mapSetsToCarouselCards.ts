...
function mapSetItemsToTiles(
  ...
  const contentType = 
    setType?.includes("trivia-multiple-choice") ? "trivia-multiple-choice" :
    setType?.includes("trivia_true_false") ? "trivia_true_false" :
    setType?.includes("factoid") ? "factoid" :
    setType?.includes("motivational") ? "motivational" :
    setType?.includes("supportive") ? "supportive" :
    setType?.includes("advisory") ? "advisory" :
    setType?.includes("slogans") ? "slogans" :
    setType?.includes("shareables") ? "shareables" :
    "unknown";
  ...
  const tiles: (HubCell | null)[] = items.slice(0, 15).map((item, index) => {
    ...
    const backgroundImage = getSequentialImage(contentType, item.theme, index);
    ...
    if (isTriviaType && item.question_text) {
      questionData = {
        ...
        question_type: item.question_type || (setType?.includes("trivia-multiple-choice") ? "multiple-choice" : "true-false"),
      };
    }
...
export function mapSetsToCarouselCards(
  ...
  return sets.map((set) => ({
    id: set.id,
    title: set.set_title,
    cells: mapSetItemsToTiles(set.set_items, set.set_type, set.id).map(cell => {
