...
export async function GET(): Promise<NextResponse> {
  try {
    ...
    // Fetch sets from aska_content_sets where set_attribution = 'Bench Boss'
    const { data, error } = await supabase
      .from("aska_content_sets")
      .select("*")
      .eq("set_attribution", "Bench Boss")
      .order("set_created_at", { ascending: false });

    if (error) {
      console.error("❌ [Bench Boss Sets API] Database error:", error);
      const errorMessage =
    ...
    console.error("💥 [Bench Boss Sets API] Unexpected error:", error);
    return NextResponse.json(
      {
        success: false,
