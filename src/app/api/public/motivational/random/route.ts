...
export async function GET(): Promise<NextResponse> {
  try {
    ...
    if (idsError) {
      console.error("Error fetching motivational IDs:", idsError);
    ...
    const { data, error } = await supabase
      .from("collection_motivational")
      .select("id, quote, context, theme, attribution")
      .eq("id", randomId)
      .eq("status", "published")
      .single();

    if (error || !data) {
      console.error("Error fetching random motivational:", error);
      return NextResponse.json(
        { success: false, error: "Failed to fetch motivational quote" },
        { status: 500 },
      );
    }
