...
The following categories belong to **Captain Heart** and should be filtered by `attribution = "Captain Heart"` (or "CaptainHeart", "Captain Hart") in the `collection_hockey_motivate` table. These are guaranteed tags that have content:
...
- Clicking a category filters items from `collection_hockey_motivate` where:
